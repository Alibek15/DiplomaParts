...
func InitDB(cfg config.Config) *gorm.DB {
	...
	if err != nil {
		log.Fatalf("Could not connect to forum database: %v", err)
	}

	if err := DB.AutoMigrate(&models.Post{}, &models.Comment{}); err != nil {
		log.Fatalf("Failed to migrate forum schema: %v", err)
	}

	log.Println("Connected to forum database and migrated schema.")
	return DB
}
